...
describe("report banding", () => {
  ...
  it("also fixes --success in the LIGHT scope, which the dark-scope pass skipped", () => {
    ...
    // report actually defaults to. --error is now ALSO declared in .report-light,
    // byte-identical to :root's value — its contrast was already fine at 5.28:1,
    // the restatement is for the both-scopes guarantee (score-track zones read
    // tokens directly off this scope, not a :root fallback). Computed OKLCH ->
    // linear sRGB -> WCAG luminance; the chosen --success value is 5.00:1 on
    // --background, 5.15:1 on --card.
