...
describe("createSubmission", () => {
  it("stores a quiz completion as a quiz completion", async () => {
    await createSubmission(BUSINESS_ID, { ...BASE, kind: "quiz", quiz_attempt_id: ATTEMPT_ID })
  ...
  it("keeps the lead when the columns do not exist yet, dropping only the label", async () => {
    ...
    const row = await createSubmission(BUSINESS_ID, { ...BASE, kind: "quiz", quiz_attempt_id: ATTEMPT_ID })
  ...
  it("does not retry an error that is not about a missing column", async () => {
    ...
    await expect(createSubmission(BUSINESS_ID, { ...BASE, kind: "quiz" })).rejects.toThrow(/foreign key/)
  ...
  it("carries the PostgREST code on the thrown error, so a duplicate is tellable from a failure", async () => {
    ...
    await createSubmission(BUSINESS_ID, { ...BASE, kind: "quiz", quiz_attempt_id: ATTEMPT_ID }).then(
