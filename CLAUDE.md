...
npm run test:integration:selects # Every PostgREST select vs the dev clone's LIVE schema (~10s, read-only)
```
...
`functions/src` and `render-worker/src` (`scripts/lib/collect-postgrest-selects.ts`) and runs each with
`limit=0`. It refuses any database but the dev clone. Two lists in the test file are ratchets, not
allowlists: `KNOWN_REFUSED` (selects production code sends today that the schema refuses — each is a bug
with its impact written down) and `KNOWN_UNRESOLVED` (selects it cannot read statically). A new entry in
either fails the run, and so does an entry that has since been fixed. The GitHub workflow of the same name
reruns it on push to `main`; that is a backstop, since it runs alongside the deploy, and it needs the
`DEV_CLONE_SERVICE_ROLE_KEY` repository secret.
