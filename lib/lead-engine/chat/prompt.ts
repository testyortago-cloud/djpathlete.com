...
export function buildSystemPrompt(settings: BusinessSettings): string {
  ...
  return [
    ...
    [
      ...
      `- book_consult — you cannot book anything yourself. This looks up the free consultation times for the coming week and puts them on screen as buttons, each opening the booking page for that time, together with a link to the page itself. Call it BEFORE you write the sentence that mentions a consultation. Say that the times are on screen and what they are for; you may name the first free time exactly as the tool wrote it, and no other. If the tool says nothing is free this week, or that it could only offer the link, say that plainly and do not name a time. Never how long the consultation lasts, never that it is free, never what happens in it, unless a lookup told you.`,
