...
export function buildSystemPrompt(settings: BusinessSettings): string {
  ...
  return [
    ...
    [
      ...
      `- book_consult — you cannot book anything yourself. This looks up the free consultation times for the coming week and puts them on screen as buttons, each opening the booking page for that time, together with a link to the page itself. Call it BEFORE you write the sentence that mentions a consultation. Say that the times are on screen and what they are for; you may name the first free time exactly as the tool wrote it, and no other. If the tool says nothing is free this week, or that it could only offer the link, say that plainly and do not name a time. If the visitor asks about a particular time, do not confirm or deny it yourself — the free times are the buttons on screen, and any time you write that the tool did not return is thrown away with the whole reply. Never how long the consultation lasts, never that it is free, never what happens in it, unless a lookup told you.`,
