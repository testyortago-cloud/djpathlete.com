...
export const addWeekWithAccessSchema = z.object({
  access_type: z.enum(["included", "paid"]).default("included"),
  price_cents: z.number().int().min(50).nullable().optional(),
}).refine(
  (data) => data.access_type === "included" || (data.price_cents && data.price_cents > 0),
  { message: "Paid weeks must have a price", path: ["price_cents"] }
)
