...
function nullableNum(schema: z.ZodTypeAny) {
  return z
    .preprocess((v) => (v == null || v === "" ? undefined : v), schema.optional())
...
export const programExerciseSchema = z.object({
  exercise_id: z
    .string()
    .regex(/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i, "Invalid exercise ID"),
  day_of_week: z.coerce.number().int().min(1).max(7),
  week_number: z.coerce.number().int().min(1),
  order_index: z.coerce.number().int().min(0),
  technique: z
    .enum(TRAINING_TECHNIQUE_OPTIONS)
    .nullable()
    .optional()
    .transform((v) => v ?? "straight_set"),
