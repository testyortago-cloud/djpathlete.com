...
export function exerciseToText(exercise: Exercise | CompressedExercise): string {
  const parts = [
    exercise.name,
    ...
    (exercise as CompressedExercise).sport_tags?.length
      ? `sports: ${(exercise as CompressedExercise).sport_tags.join(", ")}`
      : "",
    (exercise as CompressedExercise).plane_of_motion?.length
      ? `planes: ${(exercise as CompressedExercise).plane_of_motion.join(", ")}`
      : "",
  ]
...
export async function embedExercise(exercise: Exercise | CompressedExercise): Promise<number[]> {
