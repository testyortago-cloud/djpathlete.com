...
| **`scheduled_events:read` is required to SUBSCRIBE to `invitee.created`/`invitee.canceled`** — even though nothing polls for events. Proven by a 400 on the first real go-live, 2026-09-05. | Derive webhook scopes from the **event payload**, not from the calls the code makes. A scope list is a list of the data you touch. Editing an app's scopes does not upgrade an existing token — the user must reconnect to re-consent. |
