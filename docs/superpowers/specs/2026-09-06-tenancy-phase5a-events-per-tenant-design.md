...
| The paid-signup return URL: `app/api/events/[id]/checkout/route.ts` builds `baseUrl` from `process.env.NEXTAUTH_URL` and hands it to `createEventCheckoutSession` for `success_url`/`cancel_url` | One value per deployment, while every other hop in the signup path derives the tenant from the request's Host. **Not merely deferred — it becomes a real hazard the day a second tenant takes a paid signup**: a visitor on a coach's host who pays is returned to the platform's host, where `resolvePublicTenant()` answers the platform and `getEventBySlug(platformId, slug)` comes back null. Same root cause as §3.7's per-host-URL problem, and this branch made it WORSE IN KIND, not merely present: pre-branch that mismatch rendered a confirmation on the wrong domain; the new per-tenant ownership guard turns it into a hard 404 for someone who has already paid |
| The funnel catalogue staying platform-scoped while the funnel's live event rendering and its submission handling are now host-scoped | `loadCatalogues()` is correctly on `platform.ts`'s DELIBERATELY FROZEN shelf — re-scoping the whole AI builder subsystem is not this task's claim. But **this branch is what created the disagreement, and it becomes a real hazard the day a second tenant publishes a funnel with an event CTA**: the builder offers the platform's camps to a coach, the publish gate validates the chosen `eventId` against that same platform catalogue and PASSES, and the live page then renders the CTA as nothing at all — silent absence, which `lib/funnels/sections/resolve.ts`'s own file header names as the worst possible failure for an owner who cannot read the DOM. A checkout-shaped funnel publishes clean and refuses every purchase. Pre-branch both sides were unscoped and agreed with each other; resolving the disagreement belongs to phase 5b (funnels) |
