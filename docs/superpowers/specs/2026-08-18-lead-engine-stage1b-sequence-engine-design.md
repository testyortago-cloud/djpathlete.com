...
| 6 | **One-click HMAC unsubscribe** at `/unsubscribe/[token]`, plus a `List-Unsubscribe` header. | A confirm-click page defends against mail-scanner prefetch but puts friction between wanting out and being out. Extending `/api/newsletter/unsubscribe` inherits an unauthenticated raw-email POST — anyone can unsubscribe anyone — and blurs two lists. **REVISITED 2026-09-20 (ledger G07).** The token flow stays the mechanism for the link in an email; nothing here changed. But the legacy route still EXISTS and is still reachable, and it was writing `newsletter_subscribers.unsubscribed_at` and nothing else — so a person who used it kept receiving sequence email, which is the one outcome an unsubscribe exists to prevent. It now performs the same revocation, by calling the same function rather than restating it. The objection above was accepted, not dismissed: the raw-email POST is now rate-limited per IP, the consent row records the IP and user agent, and the row's `source` distinguishes `newsletter_form` from `unsubscribe_link` everywhere it is read. "Blurs two lists" is the residual cost and is deliberate — honouring an unsubscribe too broadly is the safer error. |
