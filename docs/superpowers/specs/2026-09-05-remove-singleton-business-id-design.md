...
- **Behaviour changes, all three deliberate and all tenant-correct:** the enrol route (§10.10); the Stripe capture for a known contact of a non-platform business now files under that business; quiz/submit follows the attempt. Everything else is byte-identical today because every default resolved to the same value.
