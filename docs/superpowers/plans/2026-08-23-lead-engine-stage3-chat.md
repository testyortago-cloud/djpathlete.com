...
- **`normalise()` — CORRECTED 2026-08-23 by Task 7.** Task 2 exported it from `facts.ts`, but `validate.ts` did **not** import it; it keeps its own copy so the validator stays pure (importing `facts.ts` would drag a Supabase client into a function that must never do I/O). The two genuinely differ — `validate.ts` collapses internal whitespace, `facts.ts` does not. **This is safe, but not for the reason first recorded:** they cannot disagree because `validateReply` re-normalises the grounded array it is handed with its OWN `normalise`, so both sides of every comparison run through the same function at comparison time. The safety comes from that, not from the copies being identical. Do not "fix" the divergence by importing across the boundary.
