...
| 11 | ~~**No screen to manage sequences at all** — no on/off toggle, no step editor. Turning the quiz sequences on required writing a script for the purpose.~~ **BUILT** — `/admin/sequences` has an on/off switch per row, and `/admin/sequences/[key]` carries the same switch plus a full step editor: create, reorder and remove any of the eight step kinds (including `tag` and `stage`, which nothing before this could create at all — only a migration could write one). Turning a sequence ON asks for confirmation, because it can restart people mid-sequence within minutes; turning one OFF does not, because off is always the safe direction. Before saving, it states in words what happens to anyone partway through — carries on where they are, or is stopped if their step was removed. A step that has already been sent cannot be removed (it says why: removing it would cascade `sequence_messages` away with it). Migration `00256` also gates the tick's claim on the sequence being ON, inside the RPC, before the claim increments `attempts` — see `CLAUDE.md`'s new Sequence management section for why that ordering matters. Branch `feat/sequence-management`. | Automation | ~~No~~ **Closed** | ~~3–4 days~~ done |
