...
- **FIVE BEHAVIOUR CHANGES THE OWNER WILL SEE:**
  ...
  - **(b) The contact route no longer stops when the recipients read fails.** It used to return early, which also skipped the coach's email, the visitor's auto-reply and the GHL sync. The failure is now logged and all three still run.
...
- The platform `DEFAULT` on `business_id` in 31 of 38 tenanted tables: one schema-wide decision, already known from `00278`. G31 records that the seed and capture scripts must be fixed before it can be dropped.
...
`worktree-g35-untenanted-readers`, awaiting the owner's merge. 12 are not.** Everything not
finished is Phase 4 white-label, a decision only the owner can take, or wording only the owner can
write. *(Updated 2026-09-26 on that branch: G35 is built, and the ten rows its sweep found are added
