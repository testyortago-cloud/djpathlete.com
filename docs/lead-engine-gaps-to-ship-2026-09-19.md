...
- **2026-09-26: built on branch `worktree-g47-neutral-builtin-quiz`, NOT merged.**
  ...
  - **New gate warning:** `quizGate` warns (it does not block) for each band lacking text or a link, which is exactly when `QuizRunner` hides the button. It shows in the editor's "Worth a look" and in the funnel publish review.
  - **Link rule:** now that a coach types the link, `PATCH /api/admin/quizzes/[id]` accepts only a site path (`/…`, not `//…`) or an `http(s)://` address, so `javascript:` never reaches a public `<a href>`. Every link on the dev clone passes (`/contact`, `/online`, `/assessment`, `/go/rotational-reboot-score/offer`). Production's quiz links were not read. A stored link that fails the rule would make that quiz's editor Save answer 400 with the rule's sentence, and the coach can correct it there.
  ...
  - **Verified:** 18 test files / 246 tests (the 4 changed or new + 14 that import a changed module or its consumers), 0 skipped. Seven mutants each turned tests red: no warning; warning only when both are missing; the operator's name put back; the script stops writing the link; the editor stops sending the link; the route accepts any link; the route accepts `//host`. Every file was restored and checked by sha. tsc 238/54, identical per file.
  - **Not done, and noted:** the built-in's gate body still says "the same profile we use in a full assessment", which a coach who runs no assessments should edit (not a name, so not swept). Production's quiz rows were not read.
