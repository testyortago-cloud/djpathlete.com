...
const DISPLAY_ORDER: SocialPlatform[] = [
  "facebook",
  "instagram",
  "tiktok",
  "youtube",
...
export default async function PlatformConnectionsPage() {
  ...
  const ordered = DISPLAY_ORDER.map((name) => byPlugin.get(name)).filter(
    (c): c is PlatformConnection => Boolean(c),
  )
  ...
  return (
    <div>
      ...
      <p className="text-sm text-muted-foreground mb-6">
        Activate each platform when you&apos;re ready. You can do this at any time — the AI keeps
        generating captions whether a platform is connected or not.
      </p>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {c.account_handle ?? "No account linked"}
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-3">
