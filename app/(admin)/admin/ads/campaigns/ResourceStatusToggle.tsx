...
export function ResourceStatusToggle({ endpoint, resourceKind, resourceName, initialStatus }: Props) {
  const router = useRouter()
  const [status, setStatus] = useState<GoogleAdsResourceStatus>(initialStatus)
  const [confirmOpen, setConfirmOpen] = useState(false)
  const [pending, startTransition] = useTransition()

  // REMOVED resources can only be restored from the Google Ads UI.
  if (status === "REMOVED") {
    return (
      <span
  ...
  function applyToggle() {
    ...
    startTransition(async () => {
      try {
        ...
        if (!res.ok) {
          // Resource was removed in Google Ads; the server reconciled our copy to
          // REMOVED. Reflect that immediately (the REMOVED badge replaces the toggle)
          // instead of reverting to the stale status.
          if (body.removed) {
            setStatus("REMOVED")
            toast.error(body.error ?? `This ${resourceKind} was removed in Google Ads.`)
            router.refresh()
            return
          }
          throw new Error(body.error ?? `HTTP ${res.status}`)
        }
        toast.success(`${resourceKind === "ad group" ? "Ad group" : "Ad"} ${next === "PAUSED" ? "paused" : "resumed"}.`)
        router.refresh()
      } catch (err) {
        setStatus(previous)
  ...
  return (
    <>
      <button
        ...
        disabled={pending}
        aria-label={`${verb} ${resourceKind} ${resourceName}`}
      ...
      <AlertDialog open={confirmOpen} onOpenChange={setConfirmOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {verb} this {resourceKind}?
            </AlertDialogTitle>
            <AlertDialogDescription>
              <span className="font-medium text-foreground">{resourceName}</span> will be{" "}
