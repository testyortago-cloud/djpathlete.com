...
import type { GoogleAdsAudienceType, GoogleAdsGa4Audience, GoogleAdsUserList } from "@/types/database"
...
const AUDIENCE_DESCRIPTIONS: Record<GoogleAdsAudienceType, string> = {
  ...
  icp: "Manually-curated ICP list (e.g. paid clients filtered by sport/level). Schema slot is reserved for richer signals from the AI Agent; populate manually for now via SQL or seed script.",
}
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1 max-w-2xl">
            Pushes hashed emails from your local data (bookers, newsletter subscribers) into Google Ads Customer Match
            user lists. Daily 07:00 UTC sync via <code className="font-mono text-xs">syncCustomerMatchAudiences</code>;
            only the delta since last sync is uploaded. Lists need ≥1000 active members before Google activates them for
            targeting.
          </p>
        </div>
        <SyncAudiencesButton />
      ...
      {!tokenSet ? (
        <div className="border border-warning/40 bg-warning/5 text-warning rounded-lg p-4 text-sm">
          ...
          <p className="text-xs mt-1 opacity-90">
            Each daily sync currently logs the would-be delta and skips the upload. The local mirror stays empty so the
            first post-cutover run pushes the full eligible list as additions.
          </p>
        </div>
      ) : null}

      <section>
        <h2 className="text-[11px] font-mono uppercase tracking-[0.18em] text-muted-foreground mb-3">─ User lists</h2>
        <UserListForm customerIds={customerIds} existingByType={byType} descriptions={AUDIENCE_DESCRIPTIONS} />
      </section>

      <section>
        <h2 className="text-[11px] font-mono uppercase tracking-[0.18em] text-muted-foreground mb-3">─ Sync state</h2>
        <DataTableCard>
          <DataTable>
            <DataTableHeader>
              <DataTableHead>Audience</DataTableHead>
              <DataTableHead className="w-28">Members</DataTableHead>
              <DataTableHead className="w-32">Last synced</DataTableHead>
            ...
            <tbody>
              {(["bookers", "subscribers", "icp"] as GoogleAdsAudienceType[]).map((t) => {
                ...
                if (!list) {
                  return (
                    <DataTableRow key={t} className="align-top">
                      <DataTableCell className="capitalize">{t}</DataTableCell>
                ...
                return (
                  <DataTableRow key={t} className="align-top">
                    <DataTableCell>
                      ...
                      <p className="text-xs text-muted-foreground font-mono mt-0.5">UserList {list.user_list_id}</p>
                    </DataTableCell>
                    <DataTableCell className="font-mono text-xs">{list.member_count.toLocaleString()}</DataTableCell>
                    <DataTableCell className="font-mono text-xs">{relativeTime(list.last_synced_at)}</DataTableCell>
      ...
      <p className="text-xs text-muted-foreground">
        Hashing: SHA-256 of <code className="font-mono">trim().toLowerCase()</code>. The normalized email is stored
        locally for diffing only — never uploaded. See{" "}
...
function Ga4AudiencesSection({ audiences, tokenSet }: { audiences: GoogleAdsGa4Audience[]; tokenSet: boolean }) {
  return (
    <section>
      ...
      {audiences.length === 0 ? (
        <div className="border border-dashed border-border rounded-xl p-6 bg-card text-sm text-muted-foreground space-y-3">
          <p>
            No remarketing audiences cached yet. These flow in via the GA4 ↔ Google Ads link — we can't create them via
            API; they're configured once in the Google Ads UI.
          </p>
          <div className="rounded-lg bg-surface border border-border/60 p-4 space-y-2">
            ...
            <ol className="list-decimal list-inside space-y-1 text-xs">
              <li>
                In Google Ads:{" "}
                <span className="font-mono">Tools → Setup → Linked accounts → Google Analytics (GA4)</span>. Link your
                GA4 property if not already.
              </li>
              <li>
                In Google Ads:{" "}
                <span className="font-mono">
                  Tools → Shared library → Audience manager → +New audience → Website visitors
                </span>
                . Create lists like &ldquo;Booking-page viewers&rdquo;, &ldquo;Programs-page viewers&rdquo;,
                &ldquo;Newsletter signup completers&rdquo;.
              </li>
              <li>
                In GA4: <span className="font-mono">Admin → Audiences → +New audience</span> for segments like
                &ldquo;Engaged readers&rdquo;, &ldquo;Cart abandoners&rdquo;. Toggle &ldquo;Share with Google Ads&rdquo;
                on each.
              </li>
              <li>
                Wait 24–48h for GA4 audiences to propagate, then click <strong>Sync now</strong> above to refresh this
                list.
              </li>
            </ol>
          </div>
          {!tokenSet ? (
            <p className="text-[11px] text-warning">
              The sync also requires <code className="font-mono">GOOGLE_ADS_DEVELOPER_TOKEN</code> — once that lands and
              audiences are configured, this section will populate.
            </p>
          ) : null}
        </div>
      ) : (
        <DataTableCard>
          <DataTable>
            <DataTableHeader>
              <DataTableHead>Audience</DataTableHead>
              <DataTableHead className="w-32">Type</DataTableHead>
            ...
            <tbody>
              {audiences.map((a) => (
                <DataTableRow key={a.id} className="align-top">
                  <DataTableCell>
                    <p className="font-medium text-primary leading-snug">{a.name}</p>
                    {a.description ? (
                      <p className="text-xs text-muted-foreground mt-0.5 leading-snug">{a.description.slice(0, 200)}</p>
                    ) : null}
                  ...
                  <DataTableCell align="right" className="font-mono text-xs">
                    {a.membership_life_span_days != null ? `${a.membership_life_span_days}d` : "—"}
                  </DataTableCell>
                </DataTableRow>
              ))}
            </tbody>
