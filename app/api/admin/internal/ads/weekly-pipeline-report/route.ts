// app/api/admin/internal/ads/weekly-pipeline-report/route.ts
// Internal route hit by sendWeeklyPipelineReport every Tuesday at 13:00
// UTC. Composes the funnel digest and sends via Resend to COACH_EMAIL.
// dryRun returns rendered HTML for local inspection.
//
...
import { buildWeeklyPipelineReport } from "@/lib/ads/weekly-pipeline-report"
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const report = await buildWeeklyPipelineReport({ rangeEnd })
    ...
    if (error) {
      console.error("[weekly-pipeline-report] Resend error:", error)
    ...
    console.error("[weekly-pipeline-report] Unexpected error:", err)
