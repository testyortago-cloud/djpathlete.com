...
export async function POST(request: Request) {
  try {
    ...
    if (!session?.user?.id) {
      return NextResponse.json({ error: "You must be logged in to purchase a program." }, { status: 401 })
    }
    ...
    if (!program) {
      return NextResponse.json({ error: "Program not found or is no longer available." }, { status: 404 })
    }

    // Free programs should not go through checkout
    if (program.payment_type === "free") {
      return NextResponse.json({ error: "This program is free and does not require payment." }, { status: 400 })
    }

    // Check if user has an existing assignment
    const existing = await getAssignmentByUserAndProgram(session.user.id, programId)

    // Private programs can only be purchased by assigned clients
    if (!program.is_public && !existing) {
      return NextResponse.json({ error: "This program is not available for purchase." }, { status: 403 })
    }

    if (!program.price_cents) {
      return NextResponse.json({ error: "This program does not have a price. Please contact us." }, { status: 400 })
    }
