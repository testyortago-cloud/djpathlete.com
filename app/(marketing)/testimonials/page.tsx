  ...
  return (
    <div className="flex gap-0.5" aria-label={`${rating} out of 5 stars`}>
      {Array.from({ length: 5 }).map((_, i) => (
        <Star key={i} className={`size-4 ${i < rating ? "fill-accent text-accent" : "text-border"}`} />
      ))}
    </div>
  )
}
      ...
      <section className="pt-32 pb-16 lg:pt-40 lg:pb-20 px-4 sm:px-8">
        <div className="max-w-5xl mx-auto text-center">
          ...
          <p className="text-lg text-muted-foreground max-w-2xl mx-auto leading-relaxed">
            From youth athletes to seasoned competitors — hear how DJP Athlete has helped them train smarter, recover
            faster, and reach new levels of performance.
          </p>
        </div>
      </section>
        ...
        <div className="max-w-6xl mx-auto">
          {testimonials.length > 0 ? (
            ...
            <div className="text-center py-16">
              <p className="text-lg text-muted-foreground">Testimonials are coming soon. Check back later!</p>
            </div>
          )}
        </div>
      </section>
