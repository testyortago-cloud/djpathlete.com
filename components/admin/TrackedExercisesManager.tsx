...
export function TrackedExercisesManager({
  ...
  const uniqueExercises = Array.from(new Map(programExercises.map((e) => [e.exercise_id, e])).values())
  ...
  async function handleToggle(exerciseId: string, exerciseName: string) {
    ...
    try {
      if (existing) {
        // Remove tracking
        const response = await fetch(`/api/admin/tracked-exercises/${existing.id}`, { method: "DELETE" })
        if (!response.ok) {
  ...
  async function handleMetricChange(trackedId: string, exerciseId: string, newMetric: string) {
    setLoadingIds((prev) => new Set(prev).add(exerciseId))

    try {
      // Delete old tracking entry and create new one with updated metric
      const deleteRes = await fetch(`/api/admin/tracked-exercises/${trackedId}`, { method: "DELETE" })
      ...
      startTransition(() => {
        setTracked((prev) => prev.map((t) => (t.id === trackedId ? updated : t)))
      })
    ...
    return (
      <div className="bg-white rounded-xl border border-border p-6">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h2 className="text-lg font-semibold text-primary">Tracked Exercises</h2>
        </div>
        <p className="text-sm text-muted-foreground">
          No exercises in this program yet. Add exercises to the program first, then you can select which ones to track
          for this client.
        </p>
      </div>
    )
  ...
  return (
    <div className="bg-white rounded-xl border border-border p-6">
      ...
      <p className="text-sm text-muted-foreground mb-4">
        Select which exercises to track for key lift progress and PR celebrations.
      </p>
      ...
      <div className="divide-y divide-border">
        {uniqueExercises.map((exercise) => {
          ...
          return (
            <div key={exercise.exercise_id} className="flex items-center gap-4 py-3 first:pt-0 last:pb-0">
              {/* Checkbox / Loading */}
              <div className="flex items-center shrink-0">
                {loading ? (
                  ...
                  <Checkbox
                    checked={checked}
                    onCheckedChange={() => handleToggle(exercise.exercise_id, exercise.exercise_name)}
              ...
              <div className="flex-1 min-w-0">
                <Label
                  ...
                  onClick={() => {
                    if (!loading) {
                      handleToggle(exercise.exercise_id, exercise.exercise_name)
                    }
                  }}
                >
              ...
              {checked && entry && (
                <div className="shrink-0">
                  <Select
                    ...
                    onValueChange={(value) => handleMetricChange(entry.id, exercise.exercise_id, value)}
                    disabled={loading}
                  >
