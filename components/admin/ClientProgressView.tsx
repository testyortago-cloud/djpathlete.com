...
export function ClientProgressView({
  ...
  const fetchProgressForAssignment = useCallback(
    async (assignmentId: string) => {
      ...
      setIsLoading(true)
      try {
        const res = await fetch(`/api/admin/clients/${userId}/progress?assignment_id=${assignmentId}`)
        if (!res.ok) throw new Error("Failed to fetch")
        const { data } = await res.json()

        const mapped: ProgressEntry[] = (data ?? []).map((p: Record<string, unknown>) => ({
          id: p.id as string,
          exercise_name: (p.exercises as { name?: string } | null)?.name ?? "Unknown Exercise",
  ...
  const hasAnyData = stats.totalWorkouts > 0 || achievements.length > 0 || recentProgress.length > 0
    ...
    return (
      <div className="bg-white rounded-xl border border-border p-6">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h2 className="text-lg font-semibold text-primary">Progress & Tracking</h2>
        </div>
        <p className="text-sm text-muted-foreground">
          No workout data recorded yet. Progress will appear here once the client starts logging workouts.
        </p>
      </div>
    )
  }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {STAT_CARDS.map((stat) => {
          ...
          return (
            <div key={stat.key} className="bg-white rounded-xl border border-border p-4">
              <div className="flex items-center gap-3">
                <div className={`flex items-center justify-center size-10 rounded-lg ${stat.bgColor} shrink-0`}>
                  <Icon className={`size-5 ${stat.color}`} />
                </div>
                <div>
                  <p className="text-2xl font-semibold text-foreground">{stats[stat.key]}</p>
        ...
        <div className="bg-white rounded-xl border border-border">
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-muted-foreground ml-1">({achievements.length})</span>
            </div>
            {achievementsOpen ? (
            ...
            <div className="px-6 pb-6 -mt-2">
              <div className="divide-y divide-border">
                {achievements.slice(0, 10).map((achievement) => {
                  ...
                  return (
                    <div key={achievement.id} className="flex items-center gap-3 py-3 first:pt-0 last:pb-0">
                      ...
                      <div className="shrink-0 text-right">
                        ...
                        <p className="text-xs text-muted-foreground">{formatShortDate(achievement.earned_at)}</p>
                      </div>
                    </div>
                  )
                })}
      ...
      <div className="bg-white rounded-xl border border-border">
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-semibold text-primary">Workout Logs</h2>
            <span className="text-xs text-muted-foreground ml-1">({filteredProgress.length})</span>
          </div>
          {workoutsOpen ? (
            <ChevronUp className="size-4 text-muted-foreground" />
          ) : (
        ...
        {workoutsOpen && (
          <div className="px-6 pb-6 -mt-2">
            ...
            {programs.length > 0 && (
              <div className="flex items-center gap-2 mb-4">
                <label htmlFor="program-filter" className="text-xs font-medium text-muted-foreground whitespace-nowrap">
                  Program:
                </label>
                <select
                  id="program-filter"
            ...
            {isLoading ? (
              ...
              <p className="text-sm text-muted-foreground text-center py-6">No workout logs found for this program.</p>
            ) : (
              <>
                <Table>
                  <TableHeader>
                    <TableRow className="bg-surface/50">
                      ...
                      <TableHead className="text-muted-foreground hidden sm:table-cell">Weight</TableHead>
                      <TableHead className="text-muted-foreground hidden sm:table-cell">Sets x Reps</TableHead>
                      <TableHead className="text-muted-foreground hidden md:table-cell">RPE</TableHead>
                      <TableHead className="text-muted-foreground text-right">PR</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {filteredProgress.map((entry) => {
                      const hasSetDetails = entry.set_details && entry.set_details.length > 0
                      ...
                      return (
                        <>
                          <TableRow
                            key={entry.id}
                            className={`${
                              entry.is_pr ? "bg-accent/10 hover:bg-accent/20" : ""
                            } ${hasSetDetails ? "cursor-pointer" : ""}`}
                            onClick={hasSetDetails ? () => toggleRow(entry.id) : undefined}
                          >
                            ...
                            <TableCell className="font-medium text-foreground">{entry.exercise_name}</TableCell>
                            <TableCell className="text-muted-foreground hidden sm:table-cell">
                              {formatWeight(entry.weight_kg)}
                            </TableCell>
                            <TableCell className="text-muted-foreground hidden sm:table-cell">
                              {entry.sets_completed !== null && entry.reps_completed !== null
                            ...
                            <TableCell className="text-right">
                              {entry.is_pr && (
                                <Badge className="bg-accent/20 text-accent border-accent/30 text-xs">PR</Badge>
                              )}
                            </TableCell>
                          </TableRow>
