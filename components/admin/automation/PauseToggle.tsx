...
export function PauseToggle({ initialPaused }: PauseToggleProps) {
  ...
  async function toggle() {
    ...
    try {
      ...
      toast.success(nextPaused ? "Automation paused — no scheduled runs will fire." : "Automation resumed.")
    } catch (err) {
      ...
      <div className="flex items-start gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">
            {paused
              ? "All scheduled runs (analytics, emails, voice drift, learning loop) are skipped while paused."
              : "Scheduled runs fire on their normal cadence. Flip to paused for maintenance or to silence emails temporarily."}
          </p>
          {paused && (
