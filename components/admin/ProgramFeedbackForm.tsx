...
export function ProgramFeedbackForm({ programId, onSuccess, className }: ProgramFeedbackFormProps) {
  ...
  const addIssue = useCallback(() => {
    setIssues((prev) => [...prev, { category: "", description: "", severity: "medium" }])
  }, [])

  const updateIssue = useCallback((index: number, updates: Partial<FeedbackIssue>) => {
    setIssues((prev) => prev.map((issue, i) => (i === index ? { ...issue, ...updates } : issue)))
  }, [])
  ...
  const handleSubmit = useCallback(async () => {
    ...
    setSubmitting(true)
    try {
      const validIssues = issues.filter((issue) => issue.category && issue.description.trim())
      ...
      await fetch(`/api/admin/programs/${programId}/feedback`, {
        ...
        body: JSON.stringify({
          ...
          ...(exerciseSelection > 0 ? { exercise_selection: exerciseSelection } : {}),
          ...(periodization > 0 ? { periodization } : {}),
          ...(difficultyAppropriateness > 0 ? { difficulty_appropriateness: difficultyAppropriateness } : {}),
  ...
  return (
    <div className={cn("space-y-4", className)}>
      ...
      <div className="space-y-1">
        <h4 className="text-sm font-heading font-semibold text-foreground mb-2">Program Ratings</h4>
        <StarRating value={overallQuality} onChange={setOverallQuality} label="Overall Quality" required />
        <StarRating value={balanceQuality} onChange={setBalanceQuality} label="Balance Quality" />
        <StarRating value={exerciseSelection} onChange={setExerciseSelection} label="Exercise Selection" />
        <StarRating value={periodization} onChange={setPeriodization} label="Periodization" />
        <StarRating
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          <h4 className="text-sm font-heading font-semibold text-foreground">Specific Issues</h4>
          <Button type="button" variant="outline" size="sm" className="h-7 text-xs gap-1" onClick={addIssue}>
              ...
              <div key={index} className="relative rounded-md border border-border p-3 space-y-2">
                ...
                <div className="flex items-center gap-2 pr-6">
                  <div className="flex-1">
                    ...
                    <Select value={issue.category} onValueChange={(v) => updateIssue(index, { category: v })}>
                        ...
                        {Object.entries(ISSUE_CATEGORY_LABELS).map(([value, label]) => (
                          <SelectItem key={value} value={value}>
                            <span className="flex items-center gap-1.5">
                              {value !== "other" && <AlertTriangle className="h-3 w-3 text-muted-foreground" />}
                              {label}
                            </span>
                          </SelectItem>
                        ))}
      ...
      {/* Submit */}
      {overallQuality > 0 && (
        <Button size="sm" variant="outline" className="h-7 text-xs gap-1" onClick={handleSubmit} disabled={submitting}>
