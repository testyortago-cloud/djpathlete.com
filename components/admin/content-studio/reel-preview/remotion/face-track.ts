// components/admin/content-studio/reel-preview/remotion/face-track.ts
// TWIN COPY of render-worker/src/lib/face-track.ts for the in-app <Player>
// preview. Pure trajectory math, byte-identical to the worker copy — keep in
// sync. The browser never PRODUCES a trajectory (face detection runs only in the
// worker); it only CONSUMES one via these helpers.
...
// Transform applied to the <Video> element (which is object-fit: cover).
