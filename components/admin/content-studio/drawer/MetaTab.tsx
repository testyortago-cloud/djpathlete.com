...
export function MetaTab({ video, transcript, posts }: MetaTabProps) {
  ...
  return (
    <div className="px-6 py-4 space-y-6">
      <section>
        <h3 className="font-heading text-sm text-primary uppercase tracking-wide mb-2">Upload</h3>
        {video ? (
          <dl className="rounded-lg border border-border bg-white px-4 py-2">
            ...
            <MetaRow label="Uploaded at" value={new Date(video.created_at).toLocaleString()} />
            <MetaRow label="Size" value={video.size_bytes ? `${video.size_bytes.toLocaleString()} bytes` : "—"} />
            <MetaRow label="Duration" value={video.duration_seconds ? `${video.duration_seconds}s` : "—"} />
          </dl>
        ) : (
      ...
      {transcript && (
        <section>
          <h3 className="font-heading text-sm text-primary uppercase tracking-wide mb-2">Transcript</h3>
          <dl className="rounded-lg border border-border bg-white px-4 py-2">
            <MetaRow label="Source" value={transcript.source} />
            <MetaRow label="Language" value={transcript.language} />
            <MetaRow label="AssemblyAI job id" value={transcript.assemblyai_job_id ?? "—"} />
            <MetaRow label="Created at" value={new Date(transcript.created_at).toLocaleString()} />
          </dl>
        </section>
      )}

      <section>
        <h3 className="font-heading text-sm text-primary uppercase tracking-wide mb-2">Fanout history</h3>
          ...
          <ul className="text-sm text-primary space-y-1">
            {statusHistory.map((h) => (
              <li key={h.id} className="flex items-center gap-3 py-1 border-b border-border last:border-0">
                <span className="w-24 text-xs uppercase tracking-wide text-muted-foreground">{h.platform}</span>
                <span className="flex-1">{h.status}</span>
                <span className="text-xs text-muted-foreground">{new Date(h.updated_at).toLocaleString()}</span>
              </li>
            ))}
          </ul>
        )}
      ...
      <section>
        <h3 className="font-heading text-sm text-primary uppercase tracking-wide mb-2">Publishing errors</h3>
        {failed.length === 0 ? (
