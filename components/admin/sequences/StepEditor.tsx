...
function BranchFields({
  ...
  /**
   * A chosen rule into the condition it stands for.
   *
   * EVERY MEMBER OF `BRANCH_KIND_ORDER` MUST HAVE AN ARM HERE. The `else`
   * clears the field, which is right for "Choose one…" and catastrophic for
   * anything else: the rule shows as chosen in the dropdown, the condition is
   * null underneath, and the coach is told "This split does not say which
   * people go down each side" about the rule they just picked, with no way
   * past it. G09 shipped exactly that for `clicked_last_email` and
   * `opened_last_email` — offered in the picker, labelled, and impossible to
   * choose. Only `BRANCH_KIND_LABEL` is compiler-checked; this function is
   * not, so __tests__/components/admin/sequences/StepEditor.test.tsx walks
   * every offered rule and asserts it sticks.
   */
  function setConditionKind(kind: string) {
    if (kind === "has_phone") onChange({ branch_condition: { kind: "has_phone" } })
    ...
    else if (kind === "clicked_last_email") onChange({ branch_condition: { kind: "clicked_last_email" } })
    else if (kind === "opened_last_email") onChange({ branch_condition: { kind: "opened_last_email" } })
    else if (kind === "enrolled_metadata_is")
      ...
      onChange({ branch_condition: { kind: "enrolled_metadata_is", key: "service", value: "" } })
