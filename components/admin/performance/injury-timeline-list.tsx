...
export function InjuryTimelineList({ injuries, clientUserId }: { injuries: Injury[]; clientUserId: string }) {
  if (injuries.length === 0) {
    return (
      <Card>
        <CardContent className="text-muted-foreground py-12 text-center">No injuries recorded.</CardContent>
      </Card>
    )
  }
  return (
        ...
        <ol className="divide-y">
          {injuries.map((i) => (
            <li key={i.id} className="flex items-center justify-between p-4">
              <div>
                <Link href={`/admin/clients/${clientUserId}/injuries/${i.id}`} className="font-medium hover:underline">
