    ...
    <div className="bg-white rounded-xl border border-border overflow-hidden">
      <div className="divide-y divide-border">
        {entries.map((entry) => {
          ...
          return (
            <div key={entry.id} className="px-3 sm:px-4 py-2.5 sm:py-3 flex items-center justify-between gap-3">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-1.5">
                  <p className="font-medium text-foreground text-xs sm:text-sm truncate">{entry.exercise_name}</p>
                  {entry.is_pr && (
                ...
                {parts.length > 0 && (
                  <p className="text-[10px] sm:text-xs text-muted-foreground mt-0.5">{parts.join(" · ")}</p>
                )}
              </div>
